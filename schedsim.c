...
{ 
  ...
  for (int m = 0; m < n - 1; m++) {
        for (int x = 0; x < n - x - 1; x++) {
            if (plist[x].art > plist[x + 1].art ||
                (plist[x].art == plist[x + 1].art && plist[x].bt > plist[x + 1].bt)) {
                ProcessType temp = plist[x];
                plist[x] = plist[x + 1];
                plist[x + 1] = temp;
            }
        }
    ...
    for (int m = 0; m < n; m++) {
        int minIndex = m;
        for (int x = m + 1; x < m && plist[x].art <= currentTime; x++) {
            if (plist[x].bt < plist[minIndex].bt) {
                minIndex = x;
            }
        }
        ...
        if (plist[minIndex].bt == 0) {
            currentTime++;
            plist[minIndex].bt = currentTime;
            plist[minIndex].wt = plist[minIndex].art - plist[minIndex].bt;
        }
